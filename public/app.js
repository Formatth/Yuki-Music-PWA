...
function restoreQueue() {
  ...
  document.title = `${s.title} • Yuki Music`;
...
function backupLibrary() {
  ...
  a.href = url;
  a.download = `yuki-music-backup-${new Date().toISOString().slice(0, 10)}.json`;
  a.rel = 'noopener';
  ...
  inp.onchange = () => {
    ...
    reader.onload = () => {
      try {
        ...
        if (!d || (d.app !== 'yuki-music' && d.app !== 'smw')) throw new Error('Not a Yuki Music backup');
